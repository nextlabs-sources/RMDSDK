...
static NXRMRECORD* COREAPI alloc_record(PVOID section)
{
	...
	do 
	{
		if(!phdr->hdr.ready)
		{
			std::string msg = "phdr->hdr.ready of alloc_record is false ";
		...
		do 
		{
			...
			Sleep(ulCount * 100); // Sleep for a while
		} while (ulCount < 10);

		if(!found)
		{
			std::string msg = "found of alloc_record is false ";
			OutputDebugStringA(msg.c_str());
			precord = NULL;
		}
...
std::vector<unsigned char> send_request(HANDLE  _h, unsigned long type, const void* request_data, unsigned long data_size, bool wait)
{
...
NXCONTEXT COREAPI submit_request(PVOID section, ULONG type, PVOID req, ULONG length)
{
	...
	do 
	{
		...
		if(!record)
		{
			std::string msg = "record of submit_request::alloc_record is NULL";
			OutputDebugStringA(msg.c_str());
			break;
		}
