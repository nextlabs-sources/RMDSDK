...
typedef struct _CEPermission_t {
  CEString     operation;    /**< Operation of this permission */
  CEResponse_t result;       /**< Result of policy evaluation */
  CEAttributes *obligation;  /**< Obligation */
} CEPermission_t;

typedef struct _CEPermissionsEnforcement_t {
  CEPermission_t    *permissions;
  CEint32           count;
} CEPermissionsEnforcement_t;
...
typedef struct _CEPermissionsRequest {
  CEResource *source;
  CEAttributes *sourceAttributes;
  CEResource *target;
  CEAttributes *targetAttributes;
...
#define CE_MAX_NUM_REQUESTS 250
   ...
   * \param operation (in)           Operation on the portal resource.
   ...
   * \param handle (in)              Handle from the CONN_Initialize()
   * \param operation (in)           Operation on the message
   * \param sourceFullFileName (in)  The full name of the source resource
   * \param sourceAttributes (in)    Associate attributes of the source
   * \param numRecipients (in)       The number of message recipients
   * \param recipients (in)          The string array of message recipients
   * \param ipNumber (in)            The ip address of client machine
   * \param user (in)                The user who access this file
   ...
   *
   * \brief Basic service to free a CE-type CEPermissionsEnforcement_t from the system.
   *
   * \param enforcement CE-type CEPermissionsEnforcement_t to be freed.
   */
  CEResult_t
  CEEVALUATE_FreePermissionsEnforcement(CEPermissionsEnforcement_t enforcement);
   ...
   *
   * \param cestr CE-type string to be reallocated
   * \param newstr Pointer to the new C standard string
   *
   */ 
#if defined (Linux) || defined (Darwin)
  CEResult_t
