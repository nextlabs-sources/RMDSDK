...
#include "PEPMan.h"
#include "nlthread.h"
#include "common/JavaConstants.h"
#include "CESDK_private.h"
#include "common/nlconfig.hpp"

#if defined(WIN32)
#include "common/eframework.R4.0/eframework/timer/timer_high_resolution.hpp"
#endif
...
namespace {
  ...
  CEResult_t CheckPermissionsInputs(CEHandle handle,
                                    const CEResource *source,
                                    const CEAttributes * sourceAttributes,
                                    const CEResource *target,
                                    const CEAttributes * targetAttributes,
  ...
  static void copyPermissionsEnforcementData(const CEAttributes *resultActionEnumeration,
                                             const CEAttributes_Array *policiesOrObligations,
                                             CEPermissionsEnforcement_t *destination)
  {
    if (resultActionEnumeration == NULL) {
      destination->count = 0;
      destination->permissions = NULL;
    } else {
      destination->count = resultActionEnumeration->count / 2;
      destination->permissions = new CEPermission_t[destination->count];

      for (int i = 0; i < destination->count; i++) {
        const nlchar *value = CEM_GetString(resultActionEnumeration->attrs[i * 2 + 1].value);
        const nlchar *delim1 = nlstrchr(value, _T(':'));
        const nlchar *delim2 = nlstrrchr(value, _T(':'));
        const nlstring resultStr(value, delim1);
        const nlstring operationStr(delim1 + 1, delim2);

        destination->permissions[i].operation = CEM_AllocateString(operationStr.c_str());

        if (resultStr == _T("allow")) {
          destination->permissions[i].result = CEAllow;
        } else if (resultStr == _T("deny")) {
          destination->permissions[i].result = CEDeny;
        } else {
          destination->permissions[i].result = CEDontCare;
        }

        if (policiesOrObligations->attrs_array[i * 2 + 1].count == 0) {
          destination->permissions[i].obligation = NULL;
        } else {
          destination->permissions[i].obligation = new CEAttributes;
          destination->permissions[i].obligation->count = policiesOrObligations->attrs_array[i * 2 + 1].count;
          destination->permissions[i].obligation->attrs = new CEAttribute[destination->permissions[i].obligation->count];

          for (int j = 0; j < destination->permissions[i].obligation->count; j++) {
            const nlchar *key2 = CEM_GetString(policiesOrObligations->attrs_array[i * 2 + 1].attrs[j].key);
            const nlchar *value2 = CEM_GetString(policiesOrObligations->attrs_array[i * 2 + 1].attrs[j].value);
            destination->permissions[i].obligation->attrs[j].key = CEM_AllocateString(key2);
            destination->permissions[i].obligation->attrs[j].value = CEM_AllocateString(value2);
          }
        }
      }
    }
                                       ...
                                       CEint32 timeout_in_millisec)
{
    ...
    try
    {
        ...
        CEint32 numValidRequests = 0;
        CEResult_t requestResult = CE_RESULT_SUCCESS;

        nlchar reqIDStr[64] = {0};
        ...
        for (int i = 0; i < numRequests; i++)
        {
            ...
            // the maximum eight (from, to, user, app, host, action, sendto, operating-system-user)
        ...
        char* marshaledRequest = NULL;
        static const nlchar *functionName = _T("CEEVALUATE_CheckResourcesEx");
        vector<void *> response;
        response.reserve(1024);
        if (CE_RESULT_SUCCESS == requestResult)
        {
            size_t requestLen;
            marshaledRequest = Marshal_PackReqFuncAndFormat(functionName,
                fmtString.c_str(),
                args,
                requestLen);
            ...
            CEResult_t rpcCallResult = PEPMAN_RPCCall(reqID, marshaledRequest, requestLen, handle, requestFunc, requestResult, response, timeout_in_millisec);
            ...
            // req id
            // number responses
            if (CE_RESULT_SUCCESS == rpcCallResult)
            {
 ...
 * Arguments : 
 * handle (INPUT): Handle from the CONN_Initialize()
 * operation (INPUT): Operation on the portal resource
 ...
 * app (INPUT): The application which send the message
...
/* ------------------------------------------------------------------------
 * CEEVALUATE_CheckPermissions()
 * asks Policy Controller to get the permitted operations on multi resources
 *
 * Parameters
 * handle (INPUT): handle from the CONN_Initialize().
 * requests (INPUT): an array of CEPermissionsRequests
 * numRequests (INPUT): the number of items in requests
 * additionalPQL (INPUT): A CEString of PQL that will be evaluated along with
 *  or instead of the polices in the bundle
 * ignoreBuiltinPolicies (INPUT): if true, the policies in the bundle will be
 ...
 * ------------------------------------------------------------------------
 */
CEResult_t CEEVALUATE_CheckPermissions(CEHandle handle,
                                       const CEPermissionsRequest *requests,
                                       CEint32 numRequests,
                                       CEString additionalPQL,
                                       CEBoolean ignoreBuiltinPolicies,
                                       CEint32 ipNumber,
                                       CEPermissionsEnforcement_t *enforcements,
                                       CEint32 timeout_in_millisec)
{
    ...
    try
    {
        ...
        for (int i = 0; i < numRequests; i++)
        {
            CEResult_t result = CheckPermissionsInputs(handle,
                requests[i].source, requests[i].sourceAttributes,
                requests[i].target, requests[i].targetAttributes,
                timeout_in_millisec);
            ...
            // We don't have a fixed list of dimensions, thanks to additionalAttributes. We can be clever
            // and work out just what attributes we have, but it's easier to be stupid and assume it's
            // the maximum seven (from, to, user, app, host, sendto, operating-system-user)
            // plus one for the additionalPolicies, plus whatever is in additionalAttributes
            //
            // TODO - Helper methods, so I don't have to do this manually

            int numDimensions = 7 + 1 + requests[i].numAdditionalAttributes;
        ...
        char* marshaledRequest = NULL;
        static const nlchar *functionName = _T("CEEVALUATE_CheckPermissions");
        vector<void *> response;
        response.reserve(1024);
        if (CE_RESULT_SUCCESS == requestResult)
        {
            ...
            if (CE_RESULT_SUCCESS == rpcCallResult)
            {
                for (int i = 0; i < numValidRequests; ++i)
                {
                    const CEAttributes *resultActionEnumeration = (CEAttributes *) response[i * 2 + 3];
                    const CEAttributes_Array *policiesOrObligations = (CEAttributes_Array *) response[i * 2 + 4];

                    copyPermissionsEnforcementData(resultActionEnumeration, policiesOrObligations, &enforcements[i]);
                }
            }
            else
            {
 ...
 *
 * Arguments : enf      : CE-type CEEnforcement_t to be freed
 *
 * ------------------------------------------------------------------------
 */ 
...
/* ------------------------------------------------------------------------
 * CEEVALUATE_FreePermissionsEnforcement
 *
 * Basic service to free a CE-type CEPermissionsEnforcement_t from the system
 *
 * Arguments : enf      : CE-type CEPermissionsEnforcement_t to be freed
 *
 * ------------------------------------------------------------------------
 */
CEResult_t CEEVALUATE_FreePermissionsEnforcement(CEPermissionsEnforcement_t enf)
{
  if (enf.permissions == NULL) {
    return CE_RESULT_SUCCESS;
  }

  for (int i = 0; i < enf.count; i++) {
    CEM_FreeString(enf.permissions[i].operation);

    if (enf.permissions[i].obligation != NULL) {
      for (int j = 0; j < enf.permissions[i].obligation->count; j++) {
        CEM_FreeString(enf.permissions[i].obligation->attrs[j].key);
        CEM_FreeString(enf.permissions[i].obligation->attrs[j].value);
      }
      delete[] enf.permissions[i].obligation->attrs;
      delete enf.permissions[i].obligation;
    }
  }

  delete[] enf.permissions;
 ...
 * sourceAttributesStrs (INPUT): Associate attributes of the source. This is
 *   a string array in the order of "key-1""value-1""key-2""value-2"...
 * numSourceAttributes (INPUT): number of source attributes
 * targetURL (INPUT): url of target resource
 * targetAttributesStrs (INPUT): Associate attributes of the target. This is
 *   a string array in the order of "key-1""value-1""key-2""value-2"...
 * numTargetAttributes (INPUT): number of source attributes
 ...
 * handle (INPUT): Handle from the CONN_Initialize()
 * operation (INPUT): Operation on the file
 * sourceFile (INPUT): the full name of the source resource
 * sourceAttributesStrs (INPUT): Associate attributes of the source. This is
 *   a string array in the order of "key-1""value-1""key-2""value-2"...
 * numSourceAttributes (INPUT): number of source attributes
 * targetFile (INPUT): the full name of the target resource
 * targetAttributesStrs (INPUT): Associate attributes of the target. This is
 *   a string array in the order of "key-1""value-1""key-2""value-2"...
 * numTargetAttributes (INPUT): number of source attributes
 * ipAddress (INPUT): For Sharepointe, the ip address of client machine
 * userName (INPUT): name of the user who access this file.
 * userID (INPUT): id of the user who access this file.
                                          ...
                                          CEint32 timeout_in_millisec)
{
  try {
    ...
    CEResult_t result=CSCINVOKE_CheckInputs(handle, sourceFile, targetFile,
...
/* ------------------------------------------------------------------------
 * CSCINVOKE_CEEVALUATE_CheckMsgAttachment()
 *
 * Ask the Policy Decision Point Server to evaluate the operation.
 *
 * Since .net marshaler doesn't support struct array, calling the function
 ...
 * numSourceAttributes (INPUT): number of source attributes
 * numRecipients (INPUT): the number of message recipients
 * recipientsStrs (INPUT): the string array of message recipients
 * ipAddress (INPUT): For Sharepointe, the ip address of client machine
 * userName (INPUT): name of the user who access this file.
 * userID (INPUT): id of the user who access this file.
 * userAttributesStrs (INPUT): Associate attributes of the user. This is
 *   a string array in the order of "key-1""value-1""key-2""value-2"...
 * numUserAttributes (INPUT): number of user attributes
 * appName (INPUT): name of the application that accesses the file
 * appPath (INPUT): the full path of the application. (Optional)
 * appURL (INPUT): the url that a web application is visiting.
 * appAttributesStrs (INPUT): Associate attributes of the app. This is
 *   a string array in the order of "key-1""value-1""key-2""value-2"...
 * numAppAttributes (INPUT): number of app attributes
 * performObligation (INPUT): Perform the obligation defined by the policy 
 *                            (e.g. logging / email)
 * noiseLevel (INPUT): Desirable noise level to be used for this evaluation
 * enforcement_ob (OUTPUT): Result of the policy for enforcement obligations.
 ...
 * handle (INPUT): Handle from the CONN_Initialize()
 * operation (INPUT): Operation on the file
 * sourceName (INPUT): name of source resource
 * sourceType (INPUT): type of source resource
 * sourceAttributesStrs (INPUT): Associate attributes of the source. This is
 *   a string array in the order of "key-1""value-1""key-2""value-2"...
 * numSourceAttributes (INPUT): number of source attributes
 * targetName (INPUT): name of target resource
 * targetType (INPUT): type of target resource
 * targetAttributesStrs (INPUT): Associate attributes of the target. This is
 *   a string array in the order of "key-1""value-1""key-2""value-2"...
 * numTargetAttributes (INPUT): number of target attributes
 * userName (INPUT): name of the user who access this resource.
 * userID (INPUT): id of the user who access this resource.
 * userAttributesStrs (INPUT): Associate attributes of the user. This is
 *   a string array in the order of "key-1""value-1""key-2""value-2"...
 * numUserAttributes (INPUT): number of user attributes
 * appName (INPUT): name of the application that accesses the file
    ...
    if (res == CE_RESULT_SUCCESS)
    {
      for (int i = 0; i < numRequests; i++)
      {
        ...
        else
        {
          ...
          {
            int keysize = (int) currObligation->attrs[j].key->length+1;
            obligations[i][j*2] = new nlchar[keysize];
            nlstrcpy_s(obligations[i][j*2], keysize, currObligation->attrs[j].key->buf);

            int valsize = (int) currObligation->attrs[j].value->length+1;
 ...
 *             strPtr(OUTPUT): the poter to the string that is acquired.
