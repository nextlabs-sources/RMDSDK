...
std::string wstr_to_str(const std::wstring& arg)
{
    ...
    wcstombs_s(&retval, const_cast<char*>(res.data()) , arg.length(), arg.c_str(), arg.length());
    return res;
}
...
std::wstring str_to_wstr(const std::string& arg)
{
    ...
    mbstowcs_s(&retval, const_cast<wchar_t*>(res.data()), arg.length(), arg.c_str(), arg.length());
    return res;
}
