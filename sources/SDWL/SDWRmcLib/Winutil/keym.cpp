...
RmcKeyManager::RmcKeyManager()
{
}

RmcKeyManager::RmcKeyManager(const RmcKeyManager& rhs) : _id(rhs._id), _key(rhs._key)
{
}
...
RmcKeyManager& RmcKeyManager::operator = (const RmcKeyManager& rhs)
{
    if (this != &rhs) {
        _id = rhs._id;
        _key = rhs._key;
...
SDWLResult RmcKeyManager::Create(bool machineStore)
{
    crypt::CertContext cc;
    SDWLResult res = CreateCertificate(&cc, machineStore);
    if (!res) {
        return res;
    }
...
SDWLResult RmcKeyManager::Load(bool machineStore, bool createifnone)
{
    crypt::CertContext cc;
	SDWLResult res = FindCertificate(&cc, machineStore);
    if (!res) {
		if (createifnone)
			res = CreateCertificate(&cc, machineStore);
		if (!res)
			return res;
    }
...
SDWLResult RmcKeyManager::CreateCertificate(crypt::CertContext* pcc, bool machineStore)
{
    ...
    SDWLResult res = cc.Create(client_x509_name, AT_SIGNATURE, true, machineStore, NULL, expire_time);
    if (!res) {
        return res;
    }
...
SDWLResult RmcKeyManager::FindCertificate(crypt::CertContext* pcc, bool machineStore)
{
    NX::crypt::CertSystemStore css;
	SDWLResult res = css.OpenPersonalStore(machineStore);
    if (!res) {
        return res;
    }
...
SDWLResult RmcKeyManager::GenerateIdAndKey(const std::vector<UCHAR>& key1, const std::vector<UCHAR>& key2)
{
    static const std::string sSaltSeed("nXrMCcLIENTseeD");
    UCHAR clientId[16] = { 0 };
    UCHAR clientSalt[20] = { 0 };

    SDWLResult res = crypt::CreateHmacMd5(key1.data(), (ULONG)key1.size(), key2.data(), (ULONG)key2.size(), clientId);
    if (!res) {
        return res;
    }
...
static SDWLResult get_y_from_cert_context(PCCERT_CONTEXT p, std::vector<unsigned char>& y)
{
...
SDWLResult NX::GetDHParameterY(const std::wstring& cert, std::vector<UCHAR>& y)
{
	...
	NX::crypt::CertContext cc;
	SDWLResult res = cc.Create(buf.data(), (unsigned long)buf.size());
	if (!res)
		return res;
