...
std::string build_error_msg2(const std::string& file_name, int line, const std::string& function_name, const std::string& category, const std::string format, ...);
...
#define ERROR_MSG2(c, m, ...)   NX::dbg::build_error_msg2(__FILE__, __LINE__, __FUNCTION__, c, m, __VA_ARGS__)


#define NT_STATUS_MSG(status, msg)  NX::dbg::build_error_msg("Win32", "NT Status (%08X) at %s", status, msg)
#define NT_STATUS_MSG2(status)      NX::dbg::build_error_msg2(__FILE__, __LINE__, __FUNCTION__, "Win32", "NT Status (%08X)", status)

#define HRESULT_MSG(hr, msg)        NX::dbg::build_error_msg("Win32", "HRESULT (%08X) at %s", hr, msg)
#define HRESULT_MSG2(hr)            NX::dbg::build_error_msg2(__FILE__, __LINE__, __FUNCTION__, "Win32", "HRESULT (%08X)", hr)
...
#define WIN32_ERROR_MSG2(err)       NX::dbg::build_error_msg2(__FILE__, __LINE__, __FUNCTION__, "Win32", NX::dbg::translate_win32_error(err, nullptr))
#else
#define WIN32_ERROR_MSG(err, msg)   NX::dbg::build_error_msg("Win32", NX::dbg::translate_winhttp_error(err, msg))
#define WIN32_ERROR_MSG2(err)       NX::dbg::build_error_msg2(__FILE__, __LINE__, __FUNCTION__, "Win32", NX::dbg::translate_winhttp_error(err, nullptr))
#endif
