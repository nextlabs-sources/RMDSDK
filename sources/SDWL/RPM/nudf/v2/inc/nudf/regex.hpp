...
{
    std::basic_regex<T> rgx = std::basic_regex<T>(rgx_pattern.c_str(), ignorecase?std::regex_constants::icase : std::regex_constants::ECMAScript);
...
{
    std::basic_regex<T> rgx = std::basic_regex<T>(rgx_pattern.c_str(), ignorecase?std::regex_constants::icase : std::regex_constants::ECMAScript);
    return std::regex_match(source, rgx);
}
