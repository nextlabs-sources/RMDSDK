...
public:
    ...
    service_status(const service_status& other)
    {
        dwServiceType = other.dwServiceType;
        dwCurrentState = other.dwCurrentState;
        dwControlsAccepted = other.dwControlsAccepted;
        dwWin32ExitCode = other.dwWin32ExitCode;
        dwServiceSpecificExitCode = other.dwServiceSpecificExitCode;
        dwCheckPoint = other.dwCheckPoint;
        dwWaitHint = other.dwWaitHint;
    }
    ...
    inline bool empty() const { return (0 == dwServiceType); }
    ...
    inline std::wstring status_name() const
    {
        std::wstring strstatus;
        switch (dwCurrentState)
        {
        case SERVICE_RUNNING: strstatus = L"Running"; break;
        case SERVICE_STOPPED: strstatus = L"Stopped"; break;
        case SERVICE_PAUSED: strstatus = L"Paused"; break;
        case SERVICE_START_PENDING: strstatus = L"Start pending"; break;
        case SERVICE_STOP_PENDING: strstatus = L"Stop pending"; break;
        case SERVICE_PAUSE_PENDING: strstatus = L"Pause pending"; break;
        case SERVICE_CONTINUE_PENDING: strstatus = L"Continue pending"; break;
        default: strstatus = L"Unknown"; break;
        }
        return std::move(strstatus);
    }
    ...
    service_status& operator = (const service_status& other)
    {
        if (this != &other) {
            dwServiceType = other.dwServiceType;
            dwCurrentState = other.dwCurrentState;
...
protected:
	...
	virtual bool CheckFileOpen() { return false; };
	virtual void ServiceStop(bool enable);
...
public:
    ...
    static service_status query_service_status(const std::wstring& name);
