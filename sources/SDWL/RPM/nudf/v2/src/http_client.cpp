...
void http::client::open()
{
    ...
    try {
        ...
        if (nullptr == _session_handle) {
            ...
            if (!_session_handle) {
                throw NX::exception(WIN32_ERROR_MSG2(GetLastError()), GetLastError());
            }

            // Set timeouts.
            const int milliseconds = 1000 * _timeout;
...
void http::client::send_request(const basic_request* req, basic_response* res)
{
    ...
    try {
        ...
        if (async()) {
			...
			if (WinHttpQueryOption(context->_request_handle,
				WINHTTP_OPTION_CONNECT_TIMEOUT,
				&data, &dwSize))
			{
				char buffer[256];
				memset(buffer, 0, 256);
				sprintf_s(buffer, 256, "Connection timeout: %u ms\n\n", data);
            ...
            if (WAIT_OBJECT_0 != wait_result) {
                ...
                throw NX::exception(WIN32_ERROR_MSG2(wait_error), wait_error);
            }
        }
        else {
...
void http::client::send_request_sync(basic_response* res, void* context)
{
    ...
    if (!WinHttpReceiveResponse(rc->_request_handle, nullptr)) {
        throw WIN32_ERROR_MSG2(GetLastError());
    }
...
http::string_request::string_request(const std::wstring& request_method,
    ...
    const std::vector<std::pair<std::wstring, std::wstring>>& request_headers,
    const std::vector<LPCWSTR>& accepted_types,
    const std::wstring& s) : basic_request(request_method, request_path, request_headers, accepted_types), _body(NX::conversion::utf16_to_utf8(s)), _pos(0)
{
}
...
{
    return std::move(NX::conversion::utf8_to_utf16(_body));
}
...
void http_detail::request_context::on_async_request_error(const WINHTTP_ASYNC_RESULT* result)
{
    ...
    case API_RECEIVE_RESPONSE:
        throw NX::exception(WIN32_ERROR_MSG((int)result->dwError, "WinHttpReceiveResponse"), (long)result->dwError);
    case API_QUERY_DATA_AVAILABLE:
        throw NX::exception(WIN32_ERROR_MSG((int)result->dwError, "WinHttpQueryDataAvaliable"), (long)result->dwError);
    case API_READ_DATA:
        throw NX::exception(WIN32_ERROR_MSG((int)result->dwError, "WinHttpReadData"), (long)result->dwError);
    case API_WRITE_DATA:
        throw NX::exception(WIN32_ERROR_MSG((int)result->dwError, "WinHttpWriteData"), (long)result->dwError);
    case API_SEND_REQUEST:
        throw NX::exception(WIN32_ERROR_MSG((int)result->dwError, "WinHttpSendRequest"), (long)result->dwError);
    default:
        throw NX::exception(WIN32_ERROR_MSG((int)result->dwError, "Unknown WinHTTP Function"), (long)result->dwError);
    }
}
...
void http_detail::request_context::on_async_send_complete()
{
    ...
    if (_request->get_data_length() > 0) {
        ...
        if (input == nullptr || 0 == bytes_read) {
            throw NX::exception(WIN32_ERROR_MSG2(ERROR_INVALID_DATA), ERROR_INVALID_DATA);
        }
