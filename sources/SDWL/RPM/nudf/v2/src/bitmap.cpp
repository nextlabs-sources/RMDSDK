...
using namespace NX::image;



const float NX::image::CBitmap::PI = 3.14159265f;
...
bool CBitmap::ToPNGFile(_In_ const wchar_t* file) const throw()
{
	...
	Gdiplus::Status ret = bmp.Save(file, &pngClsid, NULL);
    if (ret != 0) {
        return false;
    }
...
bool CBitmap::ToFile(_In_ const wchar_t* file, _In_opt_ LPSECURITY_ATTRIBUTES sa) const throw()
{
    HDC                 hdc = NULL;
    ...
    hFile = ::CreateFileW(file, GENERIC_READ|GENERIC_WRITE, 0, sa, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
bool CTextBitmap::Create(_In_ LPCWSTR wzText, _In_ HFONT hFont, _In_ COLORREF clrBK, _In_ COLORREF clrFont, _In_ int angle)
{
    bool    result = false;
    HDC     hScreenDC = NULL;
    HDC     hDC = NULL;
    ...
    NX::utility::Split<wchar_t>(wsText, L'\n', vText);
    if(vText.empty()) {
        return false;
    }
    ...
    const int cx_before_rotate = bmpSize.cx;
    const int cy_before_rotate = bmpSize.cy;

    angle %= 180;
    float radian = (float)((angle * CBitmap::PI) / 180.0f);
    float cosine = (float)cos(radian);
    float sine = (float)sin(radian);
    if (0 != angle) {
        // Compute dimensions of the resulting bitmap
        // First get the coordinates of the 3 corners other than origin
        int x1 = (int)(bmpSize.cy * sine);
        int y1 = (int)(bmpSize.cy * cosine);
        int x2 = (int)(bmpSize.cx * cosine + bmpSize.cy * sine);
        int y2 = (int)(bmpSize.cy * cosine - bmpSize.cx * sine);
        int x3 = (int)(bmpSize.cx * cosine);
        int y3 = (int)(-bmpSize.cx * sine);
        ...
        bmpSize.cx = maxx - minx;
        bmpSize.cy = maxy - miny;
    }
    ...
    for(std::vector<std::pair<std::wstring, SIZE>>::const_iterator it=vTextInfo.begin(); it!=vTextInfo.end(); ++it) {
        ...
        int text_y = 0;
        if (0 == angle) {
            text_x = (bmpSize.cx - (*it).second.cx) / 2;
            text_y = orig_y;
        }
        else {
            text_x = (int)((bmpSize.cx - (cosine * (*it).second.cx + sine * (*it).second.cy)) / 2);
            text_y = (int)(((*it).second.cx * sine) / 2 + bmpSize.cy / 2 - (cy_before_rotate / 2 - orig_y) * cosine);
        }
        TextOutW(hDC, text_x, text_y, (*it).first.c_str(), (int)(*it).first.length());
        orig_y += (nLineSpace + (*it).second.cy);
    }
...
bool CTextBitmap::Create(_In_ LPCWSTR wzText, _In_ LPCWSTR wzFont, _In_ int nFontSize, _In_ COLORREF clrBK, _In_ COLORREF clrFont, _In_ int angle)
{
    bool  result = false;
    HFONT hFont;
    ...
    hFont = CreateFontW( nHeight, 0,
                         angle * 10, angle * 10,
                         FW_BOLD, 0, FALSE, FALSE,
                         ANSI_CHARSET,
                         OUT_DEFAULT_PRECIS,
                         CLIP_DEFAULT_PRECIS,
                         PROOF_QUALITY,
    ...
    result = Create(wzText, hFont, clrBK, clrFont, angle);
    ::DeleteObject(hFont);
    hFont = NULL;
