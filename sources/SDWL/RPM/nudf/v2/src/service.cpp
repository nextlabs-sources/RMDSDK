...
static service_instance* instance_ptr = nullptr;
...
//service_instance::service_instance() : _ctls_accepted(SERVICE_ACCEPT_DEFAULT_CTLS), _h(NULL), _stop_event(NULL), _stop_hint(0), _dev_notify_h(NULL)
//{
//}

service_instance::service_instance(const std::wstring& name, unsigned long ctls_accepted) : _name(name), _ctls_accepted(ctls_accepted), _h(NULL), _stop_event(NULL), _stop_hint(0), _dev_notify_h(NULL)
{
}
...
void service_instance::clear()
{
    _status.clear();
    _ctls_accepted = SERVICE_ACCEPT_DEFAULT_CTLS;
    _stop_hint = 0;
    _h = NULL;
    ...
    if (NULL != _dev_notify_h) {
        UnregisterDeviceNotification(_dev_notify_h);
        _dev_notify_h = NULL;
    }
}
...
long service_instance::start(int argc, const wchar_t** argv) noexcept
{
    ...
    try {
		...
		// disable pause & stop service
		_status.dwControlsAccepted &= ~SERVICE_ACCEPT_STOP;
		_status.dwControlsAccepted &= ~SERVICE_PAUSED;
...
void service_instance::ServiceStop(bool enable)
{
	// disable pause & stop service
	if (enable)
		_status.dwControlsAccepted |= SERVICE_ACCEPT_STOP;
...
void service_instance::stop() noexcept
{
    ...
    set_status(SERVICE_STOP_PENDING, NO_ERROR, 2000);
...
service_control::service_control() : _h(NULL), _h_mgr(NULL)
{
}
...
bool service_control::exists(const std::wstring& name)
{
    ...
    h_mgr = ::OpenSCManagerW(NULL, NULL, GENERIC_READ);
    if (NULL == h_mgr) {
        return false;
    }

    h = ::OpenServiceW(h_mgr, name.c_str(), SERVICE_QUERY_STATUS);
    result = (NULL != h);
    if (NULL != h) { CloseServiceHandle(h); h = NULL; }
    CloseServiceHandle(h_mgr); h_mgr = NULL;
...
service_status service_control::query_service_status(const std::wstring& name)
{
    bool result = false;
...
{
    const unsigned long service_desired_access = read_only ? (STANDARD_RIGHTS_REQUIRED | SERVICE_QUERY_CONFIG | SERVICE_QUERY_STATUS | SERVICE_ENUMERATE_DEPENDENTS) : (DELETE | SERVICE_ALL_ACCESS);
    const unsigned long service_mgr_desired_access = read_only ? (SC_MANAGER_CONNECT | SC_MANAGER_ENUMERATE_SERVICE) : SC_MANAGER_ALL_ACCESS;
