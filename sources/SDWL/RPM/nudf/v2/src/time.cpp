...
time::datetime::datetime(datetime&& other) : _t(std::move(other._t))
{
}
...
time::datetime& time::datetime::operator = (time::datetime&& other)
{
    if (this != &other) {
        _t = other._t;
        other._t = 0;
...
{
    return  std::move(time::datetime((dos_time * 10000000) + TIME_DIFF));
}

__int64 time::datetime::to_dos_time()
{
    __int64 dos_time = 0;
    if (_t > TIME_DIFF) {
...
time::datetime time::datetime::from_java_time(__int64 dos_time)
{
    return  std::move(time::datetime((dos_time * 10000) + TIME_DIFF));
}

__int64 time::datetime::to_java_time()
{
    __int64 java_time = 0;
    if (_t > TIME_DIFF) {
...
std::wstring time::datetime_rfc1123::serialize(bool local_time, bool with_milliseconds) const
{
    ...
    if (local_time) {
        ...
        s = NX::string_formater(L"%s, %02d %s %04d %02d:%02d:%02d %s",
                                (st.wDayOfWeek < 0 || st.wDayOfWeek > 6) ? "Ukn" : time_detail::utf8_weekday_names[st.wDayOfWeek],
                                st.wDay,
...
std::wstring time::datetime_special_1::serialize(bool local_time, bool with_milliseconds) const
{
    ...
    return NX::string_formater( L"%s %d, %04d %02d:%02d:%02d %s",
                                (st.wMonth < 1 || st.wMonth > 12) ? L"Ukn" : time_detail::utf16_month_names[st.wMonth - 1],
