...
public:
    ...
    public:
        ...
        inline void* get_request() const noexcept { return (void*)(_request.empty() ? NULL : _request.data()); }
        inline void* get_context() const noexcept { return _context; }
        inline unsigned long get_type() const noexcept { return _type; }
...
public:
    ...
    std::wstring drvctl_query_process_info(unsigned long process_id) noexcept;
    void drvctl_set_overlay_windows(unsigned long session_id, const std::vector<unsigned long>& hwnds);
    void drvctl_set_overlay_windows2(unsigned long session_id);
