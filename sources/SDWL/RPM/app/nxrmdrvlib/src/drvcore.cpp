...
#define ID_SET_OVERLAY_BITMAP_STATUS                    202



drvcore_man_instance::drvcore_man_instance() : dll_instance<DRVMAN_FUNCTION_NUMBER>(std::vector<function_item>({
    ...
    function_item(ID_SET_OVERLAY_BITMAP_STATUS, "SetOverlaySetStatus")
}))
{
}
...
void drvcore_man_instance::load(const std::wstring& dll_file)
{
    ...
    if (!is_loaded()) {
        throw NX::exception(NX::string_formater("Fail to load nxrmdrvman.dll (%S)", dll_file.c_str()));
    }

    std::for_each(begin(), end(), [](const std::pair<unsigned long, function_item>& item) {
        if (nullptr == item.second.get_function_pointer()) {
            throw NX::exception(NX::string_formater("Cannot find function %S in nxrmdrvman.dll", item.second.get_name().c_str()));
        }
    });
}
...
unsigned long drvcore_man_instance::post_update_dwm_window_response(HANDLE h, void* context)
{
    typedef unsigned long (WINAPI* NXRMDRV_POST_QUERY_UPDATE_DWM_WINDOW_RESPONSE)(HANDLE, void*);
    return EXECUTE(NXRMDRV_POST_QUERY_UPDATE_DWM_WINDOW_RESPONSE, *this, ID_POST_QUERY_UPDATE_DWM_WINDOW_RESPONSE, h, context);
}

unsigned long drvcore_man_instance::post_update_overlay_window_response(HANDLE h, void* context)
{
    typedef unsigned long (WINAPI* NXRMDRV_POST_QUERY_UPDATE_OVERLAY_WINDOW_RESPONSE)(HANDLE, void*);
    return EXECUTE(NXRMDRV_POST_QUERY_UPDATE_OVERLAY_WINDOW_RESPONSE, *this, ID_POST_QUERY_UPDATE_OVERLAY_WINDOW_RESPONSE, h, context);
}
...
unsigned long drvcore_man_instance::set_overlay_bitmap_status(HANDLE h, unsigned long session_id, bool ready)
{
    ...
    return EXECUTE(NXRMDRV_SET_OVERLAY_BITMAP_STATUS, *this, ID_SET_OVERLAY_BITMAP_STATUS, h, session_id, ready ? TRUE : FALSE);
}
