...
namespace nx {
	...
	void cregistry_whitelist::load_whitelist_from_registry()
	{
        ...
        ::OutputDebugStringW(m_strAppWhiteListName.c_str());
	...
	std::wstring cregistry_whitelist::get_whitelist_itemname(const std::wstring& strAppPath)
	{
        ...
        std::wstring strItemName = gen_md5_hex_string(appPath);//appName;// +L":" + strAppPathHash;
		return strItemName;
	}
	...
	std::wstring cregistry_whitelist::get_process_path(DWORD ProcessId)
	{
		...
		do
		{
			//hProcess = ::OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, ProcessId); //PROCESS_QUERY_INFORMATION | PROCESS_VM_READ
