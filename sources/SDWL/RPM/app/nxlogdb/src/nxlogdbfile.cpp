...
public:
    ...
    void export_conf(const db_conf& conf)
    {
        ...
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    Description: %S", conf.get_description().c_str());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB Size: %d MB", conf.get_db_size() / 1048576);
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB Flags: %08X", conf.get_db_flags());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB Block Size: %d", conf.get_block_size());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB Sector Size: %d", conf.get_sector_size());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB Sector data Size: %d", conf.get_sector_data_size());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB Sector fields Size: %d", conf.get_fields_size());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB record map offset: 0x%08X", conf.get_record_map_offset());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB record map size: 0x%08X", conf.get_record_map_size());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB record start id: 0x%08X", conf.get_record_start_id());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB record count: 0x%08X", conf.get_record_count());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB record max count: 0x%08X", conf.get_max_record_count());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB sector map offset: 0x%08X", conf.get_sector_map_offset());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB sector map size: 0x%08X", conf.get_sector_map_size());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB sector count: 0x%08X", conf.get_sector_start_offset());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"    DB sector start offset: 0x%08X", conf.get_max_record_count());
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();

        const NXLOGDB::record_layout& layout = conf.get_record_layout();
        for (int i = 0; i < (int)layout.get_fields_def().size(); i++) {
            swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"      > Field #%d", i);
            write_file(line.c_str(), line.length() * sizeof(wchar_t));
            write_line_ending();
            swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"          Name:  %S", layout.get_fields_def()[i].get_field_name().c_str());
            write_file(line.c_str(), line.length() * sizeof(wchar_t));
            write_line_ending();
            swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"          Type:  %s", layout.get_fields_def()[i].get_field_type_name());
            write_file(line.c_str(), line.length() * sizeof(wchar_t));
            write_line_ending();
            swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"          Flags: %08X", layout.get_fields_def()[i].get_field_flags());
            write_file(line.c_str(), line.length() * sizeof(wchar_t));
            write_line_ending();
            swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"          Size:  %d", layout.get_fields_def()[i].get_field_size());
            write_file(line.c_str(), line.length() * sizeof(wchar_t));
            write_line_ending();
        }
    ...
    void export_record(unsigned long id, const db_record& record, const record_layout& layout)
    {
        ...
        swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"  Record #%d", id);
        write_file(line.c_str(), line.length() * sizeof(wchar_t));
        write_line_ending();

        for (int i = 0; i < (int)layout.get_fields_def().size(); i++) {
            // write name
            swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"      %S: ", layout.get_fields_def()[i].get_field_name().c_str());
            write_file(line.c_str(), line.length() * sizeof(wchar_t));
            // write value
            const LOGDB_FIELD_TYPE record_field_type = record.get_fields()[i].get_type();
            const std::string& record_field_name = layout.get_fields_def()[i].get_field_name();
            ...
            case FIELD_UNSIGNED_INTEGER:
                {
                    ...
                    if (is_time) {
                        NX::time::datetime tm((__int64)u);
                        line = tm.serialize(true, true);
                    }
                    else if (0 == _stricmp(record_field_name.c_str(), "result")) {
                        line = (1 ==u) ? L"Allow" : L"Deny";
                    }
                    else if (0 == _stricmp(record_field_name.c_str(), "operation")) {
                        switch (u)
                        {
                        case 1: line = L"Protect"; break;
                        case 2: line = L"Share"; break;
                        case 3: line = L"Modify Share"; break;
                        case 4: line = L"View"; break;
                        case 5: line = L"Print"; break;
                        case 6: line = L"Download"; break;
                        case 7: line = L"Edit"; break;
                        case 8: line = L"Revoke"; break;
                        case 9: line = L"Decrypt"; break;
                        case 10: line = L"Copy Content"; break;
                        case 11: line = L"Capture Screen"; break;
                        case 12: line = L"Classify"; break;
                        default: swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"Unknown (%d)", (int)u); break;
                        }
                    }
                    else if (0 == _stricmp(record_field_name.c_str(), "device_type")) {
                        switch (u)
                        {
                        case 0: line = L"Windows"; break;
                        case 1: line = L"Windows 2000"; break;
                        case 2: line = L"Windows XP"; break;
                        case 3: line = L"Windows Vista"; break;
                        case 4: line = L"Windows 7"; break;
                        case 5: line = L"Windows 8"; break;
                        case 6: line = L"Windows 8.1"; break;
                        case 7: line = L"Windows 10"; break;
                        case 100: line = L"Windows Server"; break;
                        case 101: line = L"Windows Server 2003"; break;
                        case 102: line = L"Windows Server 2003 R2"; break;
                        case 103: line = L"Windows Server 2008"; break;
                        case 104: line = L"Windows Server 2008 R2"; break;
                        case 105: line = L"Windows Server 2012"; break;
                        case 106: line = L"Windows Server 2012 R2"; break;
                        case 107: line = L"Windows Server 2016"; break;
                        default: swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"Unknown %d", (int)u); break;
                        }
                    }
                    else {
                        if (FIELD_INTEGER == record_field_type) {
                            swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"%I64d", record.get_fields()[i].as_int64());
                        }
                        else {
                            if (u > 0xFFFFFFFF) {
                                swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"0x%08X%08x", (unsigned long)(u >> 32), (unsigned long)u);
                            }
                            else {
                                swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, L"0x%08X", (unsigned long)u);
                            }
                        }
                    }
                }
            ...
            case FIELD_BOOL:
                swprintf_s(NX::string_buffer<wchar_t>(line, MAX_PATH), MAX_PATH, record.get_fields()[i].as_boolean() ? L"true" : L"false");
                break;
        ...
        if (!opaque_data.empty()) {
            line = L"      Opaque Data: ";
            line += NX::conversion::utf8_to_utf16(std::string(opaque_data.begin(), opaque_data.end()));
        }
        else {
            line = L"      Opaque Data: ";
        }
...
{
public:
    log_exporter2(const std::wstring& file) : _h(INVALID_HANDLE_VALUE)
    {
        open(file);
    }
    virtual ~log_exporter2()
    {
        close();
    }
    ...
    void export_record(bool is_first, const db_record& record, const record_layout& layout)
    {
        std::string line;
        ...
        try {
            ...
            for (int i = 0; i < (int)layout.get_fields_def().size(); i++) {
                ...
                const LOGDB_FIELD_TYPE record_field_type = record.get_fields()[i].get_type();
                const std::wstring& record_field_name = NX::conversion::utf8_to_utf16(layout.get_fields_def()[i].get_field_name());

                if (0 == _wcsicmp(record_field_name.c_str(), L"timestamp")) {
                    NX::time::datetime tm(record.get_fields()[i].as_int64());
                    const std::wstring& tmstr = tm.serialize(true, true);
                    v[L"timestamp"] = NX::json_value(tmstr.c_str());
                }
                else if (0 == _wcsicmp(record_field_name.c_str(), L"operation")) {
                    v[L"operation"] = NX::json_value(ActivityToDisplayName(record.get_fields()[i].as_uint64()));
                }
                else if (0 == _wcsicmp(record_field_name.c_str(), L"result")) {
                    const unsigned __int64 u = record.get_fields()[i].as_uint64();
                    v[L"result"] = NX::json_value(u ? L"Allowed" : L"Denied");
                }
                else if (0 == _wcsicmp(record_field_name.c_str(), L"duid")) {
                    const std::wstring& duid = record.get_fields()[i].as_string();
                    v[L"duid"] = NX::json_value(duid.c_str());
                }
                else if (0 == _wcsicmp(record_field_name.c_str(), L"owner")) {
                    const std::wstring& owner = record.get_fields()[i].as_string();
                    v[L"owner"] = NX::json_value(owner.c_str());
                }
                else if (0 == _wcsicmp(record_field_name.c_str(), L"file_path")) {
                    const std::wstring& file_path = record.get_fields()[i].as_string();
                    v[L"file"] = NX::json_value(file_path.c_str());
                }
                else if (0 == _wcsicmp(record_field_name.c_str(), L"app_name")) {
                    const std::wstring& app_name = record.get_fields()[i].as_string();
                    v[L"application"] = NX::json_value(app_name.c_str());
                }
                else {
                    // Nothing
                }
            ...
            line = NX::conversion::utf16_to_utf8(ws);
        }
        ...
        if (!line.empty()) {
            ...
            write_file(line.c_str(), line.length());
        }
    }
...
void logdb_file::create(const std::wstring& file, const db_conf& conf)
{
    ...
    if (opened()) {
        DBGLOG(LL_ERROR, L"db file has been opened already");
        throw std::exception("db already opened");
    }

    if (!NXLOGDB::db_conf_validate(conf)) {
        DBGLOG(LL_ERROR, L"invalid db configuration");
    ...
    if (INVALID_HANDLE_VALUE == _h) {
        DBGLOG(LL_ERROR, L"fail to create db file (%s), error is %d", file.c_str(), GetLastError());
    ...
    try {
        ...
        if (!set_file_size(_db_conf.get_sector_start_offset())) {
            DBGLOG(LL_ERROR, L"fail to initialize db file size (%s), error is %d", file.c_str(), GetLastError());
            throw std::exception("fail to create db file");
        }
        ...
        if (!write_file(0, &header, (unsigned long)sizeof(header))) {
            DBGLOG(LL_ERROR, L"ERROR: fail to save initial db file header (%s), error is %d", file.c_str(), GetLastError());
            throw std::exception("fail to create db file");
        }

        // allocate first page
        if (!allocate_block()) {
            DBGLOG(LL_ERROR, L"ERROR: fail to allocate first page in db file (%s), error is %d", file.c_str(), GetLastError());
            throw std::exception("fail to create db file");
        }
...
void logdb_file::open(const std::wstring& file, bool read_only)
{
    ...
    if (INVALID_HANDLE_VALUE == _h) {
        DBGLOG(LL_ERROR, L"fail to open db file (%s), error is %d", file.c_str(), GetLastError());
    ...
    try {
        ...
        if (!read_file(0, &header, sizeof(header))) {
            DBGLOG(LL_ERROR, L"fail to read header from db file (%s), error is %d", file.c_str(), GetLastError());
            throw std::exception("fail to read header from db file");
        }

        read_conf(&header);
        if (_db_conf.empty()) {
            DBGLOG(LL_ERROR, L"invalid db header");
        ...
        if (!read_file(_db_conf.get_sector_map_offset(), map_buffer.data(), _db_conf.get_sector_map_size())) {
            DBGLOG(LL_ERROR, L"fail to read sector map from db file (%s), error is %d", file.c_str(), GetLastError());
        ...
        if (!read_file(_db_conf.get_record_map_offset(), map_buffer.data(), _db_conf.get_record_map_size())) {
            DBGLOG(LL_ERROR, L"fail to read record map from db file (%s), error is %d", file.c_str(), GetLastError());
...
void logdb_file::export_log(const std::wstring& file) const
{
    ...
    if (!exporter.is_opened()) {
        DBGLOG(LL_ERROR, L"Fail to open db log export file (%d)", GetLastError());
        return;
    }
...
void logdb_file::export_log2(const std::wstring& file, bool descending) const
{
    log_exporter2 exporter(file);
    shared_locker lock_db(&_db_lock);
    const unsigned long max_record = get_db_conf().get_record_count();
    if (0 == max_record) {
        return;
    }
    const unsigned long begin_index = descending ? (max_record - 1) : 0;
    const unsigned long end_index = descending ? 0 : (max_record -1);

    if (!descending) {
        for (unsigned long i = 0; i < max_record; i++) {
            try {
                const NXLOGDB::db_record& record = read_record_safe(i);
                exporter.export_record((0 == i), record, get_db_conf().get_record_layout());
            }
            catch (const std::exception& e) {
                UNREFERENCED_PARAMETER(e);
                DBGLOG(LL_ERROR, L"Fail to open db log export file (%d)", GetLastError());
        ...
        for (unsigned long i = max_record; i != 0; i--) {
            try {
                const NXLOGDB::db_record& record = read_record_safe(i - 1);
                exporter.export_record((i == max_record), record, get_db_conf().get_record_layout());
            }
            catch (const std::exception& e) {
                UNREFERENCED_PARAMETER(e);
                DBGLOG(LL_ERROR, L"Fail to open db log export file (%d)", GetLastError());
...
NXLOGDB::db_record logdb_file::read_record_safe(unsigned long record_id) const
{
    if (get_db_conf().record_empty() || record_id >= get_db_conf().get_record_count()) {
        DBGLOG(LL_ERROR, L"record #%d not found", record_id);
    ...
    if (0 == sector_id) {
        DBGLOG(LL_ERROR, L"record #%d is empty", record_id);
    ...
    if ((get_db_conf().get_fields_size() + 4) > (unsigned long)data.size()) {
        DBGLOG(LL_ERROR, L"Not read all the fields of record #%d (read: %d, expected: %d)", record_id, data.size(), get_db_conf().get_fields_size() + 4);
    ...
    if ((get_db_conf().get_fields_size() + 4 + opaque_data_size) > (unsigned long)data.size()) {
        DBGLOG(LL_ERROR, L"Not read all the fields of record #%d (read: %d, expected: %d)", record_id, data.size(), get_db_conf().get_fields_size() + 4 + opaque_data_size);
...
bool logdb_file::push_record(const NXLOGDB::db_record& record)
{
    ...
    if (!inter_write_record(record_start_sector_id, buf)) {
        ...
        DBGLOG(LL_ERROR, L"fail to write record #%d (start sector id = %d)", next_reord_id, record_start_sector_id);
        return false;
    }
...
bool logdb_file::write_record_count(unsigned long count)
{
    if (count >= get_db_conf().get_max_record_count()) {
        ...
        DBGLOG(LL_ERROR, L"record (%d) exceed max count (%d)", count, get_db_conf().get_max_record_count());
        return false;
    }
...
bool logdb_file::write_record_start_id(unsigned long record_id)
{
    if (record_id >= get_db_conf().get_max_record_count()) {
        SetLastError(ERROR_INVALID_PARAMETER);
        DBGLOG(LL_ERROR, L"record id (%d) exceed max count (%d)", record_id, get_db_conf().get_max_record_count());
        return false;
    }
...
bool logdb_file::write_record_sector_id(unsigned long record_id, unsigned long sector_id)
{
    ...
    if (sector_id >= get_db_conf().get_sector_count() || 0 == sector_id) {
        SetLastError(ERROR_INVALID_PARAMETER);
        DBGLOG(LL_ERROR, L"sector id (%d) exceed max count (%d) or sector is zero", sector_id, get_db_conf().get_sector_count());
        return false;
    }
...
bool logdb_file::write_sector_next_id(unsigned long sector_id, unsigned long next_sector_id)
{
    if (sector_id >= get_db_conf().get_sector_count() || next_sector_id >= get_db_conf().get_sector_count()) {
        SetLastError(ERROR_INVALID_PARAMETER);
        DBGLOG(LL_ERROR, L"sector id (%d, %d) exceed max count (%d)", sector_id, next_sector_id, get_db_conf().get_sector_count());
        return false;
    }
...
bool logdb_file::write_sector_map_block(unsigned long block_id, unsigned long block_value)
{
    if (block_id >= _sector_map.block_count()) {
        SetLastError(ERROR_INVALID_PARAMETER);
        DBGLOG(LL_ERROR, L"sector map block id (%d) exceed max count (%d)", block_id, _sector_map.block_count());
        return false;
    }
...
bool logdb_file::allocate_block() noexcept
{
    ...
    if (current_data_size >= get_db_conf().get_db_size()) {
        DBGLOG(LL_CRITICAL, L"DB reach max size (%d)", get_db_conf().get_db_size());
        return false;
    }

    if (!IS_ALIGNED(current_data_size, get_db_conf().get_block_size())) {
        assert(false);
        DBGLOG(LL_CRITICAL, L"Data size (%d) is not aligned with page size (%d)", current_data_size, get_db_conf().get_block_size());
        return false;
    }

    if (!set_file_size(new_file_size)) {
        DBGLOG(LL_CRITICAL, L"Fail to set file size (%d), error is %d", new_file_size, GetLastError());
        return false;
    }
...
void logdb_file::read_conf(const DBHEADER* header)
{
    ...
    if (!NXLOGDB::db_conf_validate(_db_conf)) {
        _db_conf.clear();
        DBGLOG(LL_ERROR, L"Invalid logdb_info");
    }
}
