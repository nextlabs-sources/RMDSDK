...
void DBGLOG(unsigned long level, const wchar_t* message, ...)
{
    if (nullptr != GlobalDbgLogFunc && DBGLOGACCEPT(level)) {
        ...
        vswprintf_s(NX::string_buffer<wchar_t>(s, len), len, message, args); // C4996
        va_end(args);
...
{
    DBGLOG(LL_DEBUG, L"Validate logdb_ifno");

    if (conf.empty()) {
        DBGLOG(LL_DEBUG, L"  --> Empty");
        return false;
    }

    if (0 != (conf.get_block_size() % ONE_MEGA_BYTES)) {
        DBGLOG(LL_DEBUG, L"  --> Invalid block_size (%d)", conf.get_block_size());
        return false;
    }

    if (conf.get_db_size() > ONE_GIGA_BYTES) {
        DBGLOG(LL_DEBUG, L"  --> Invalid db_size (%d)", conf.get_db_size());
        return false;
    }
    ...
    if (conf.get_sector_size() != SECTOR_512 && conf.get_sector_size() != SECTOR_1024 && conf.get_sector_size() != SECTOR_2048 && conf.get_sector_size() != SECTOR_4096) {
        DBGLOG(LL_DEBUG, L"  --> Invalid sector_size (%d)", conf.get_sector_size());
        return false;
    }

    if (conf.get_record_layout().get_fields_def().size() > MAX_FIELDS_NUMBER) {
        DBGLOG(LL_DEBUG, L"  --> Invalid fields number (%d)", conf.get_record_layout().get_fields_def().size());
        return false;
    }
    ...
    if (expected_sector_count != conf.get_sector_count()) {
        DBGLOG(LL_DEBUG, L"  --> Invalid sector_count (%d, expected %d)", conf.get_sector_count(), expected_sector_count);
        return false;
    }
    if (expected_fields_size != conf.get_fields_size()) {
        DBGLOG(LL_DEBUG, L"  --> Invalid field_size (%d, expected %d)", conf.get_fields_size(), expected_fields_size);
        return false;
    }
    if (expected_record_map_size != conf.get_record_map_size()) {
        DBGLOG(LL_DEBUG, L"  --> Invalid record_map_size (%d, expected %d)", conf.get_record_map_size(), expected_record_map_size);
        return false;
    }
    if (expected_sector_map_size != conf.get_sector_map_size()) {
        DBGLOG(LL_DEBUG, L"  --> Invalid sector_map_size (%d, expected %d)", conf.get_sector_map_size(), expected_sector_map_size);
        return false;
    }
    if (expected_record_map_offset != conf.get_record_map_offset()) {
        DBGLOG(LL_DEBUG, L"  --> Invalid record_map_offset (%d, expected %d)", conf.get_record_map_offset(), expected_record_map_offset);
        return false;
    }
    if (expected_sector_map_offset != conf.get_sector_map_offset()) {
        DBGLOG(LL_DEBUG, L"  --> Invalid sector_map_offset (%d, expected %d)", conf.get_sector_map_offset(), expected_sector_map_offset);
        return false;
    }
    if (expected_sector_start_offset != conf.get_sector_start_offset()) {
        DBGLOG(LL_DEBUG, L"  --> Invalid sector_start_offset (%d, expected %d)", conf.get_sector_start_offset(), expected_sector_start_offset);
        return false;
    }

    DBGLOG(LL_DEBUG, L"  --> Succeed");
    return true;
}
...
bool nxlogdb::opened() const
{
    return _dbfile->opened();
}
...
void nxlogdb::export_log2(const std::wstring& file, bool descending) const
{
    _dbfile->export_log2(file, descending);
}