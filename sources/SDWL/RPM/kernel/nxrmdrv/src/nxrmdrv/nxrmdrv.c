...
BOOLEAN				g_bDWMAttached = TRUE; //flag to use tracking if dwm is attached when driver starts

//
// Device driver routine declarations.
//
DRIVER_INITIALIZE DriverEntry;
DRIVER_UNLOAD DummyUnload;///ayuen remove
...
#define MAX_PATH    260
static WCHAR nxLogFileName[MAX_PATH];     // file name under %SystemRoot%\Temp

BOOLEAN nxSetLogFileName(__in PCWCHAR LogFileName)
{
	return wcscpy_s(nxLogFileName, MAX_PATH, LogFileName) == 0;
}
...
BOOLEAN nxWriteToLogFile(__in PWCHAR Msg)
{
	...
	status = ZwWriteFile(h, NULL, NULL, NULL, &ioStatus, Msg, (ULONG) (wcslen(Msg) * sizeof(WCHAR)), NULL, NULL);
	...
	)
{
	...
	nxSetLogFileName(L"nxrmxxx.log");
	nxWriteToLogFile(L"nxrmdrv!DriverEntry: Enter");
	...
	DriverObject->DriverUnload							= DummyUnload;///ayuen keep NULL;
	...
	if (NT_SUCCESS(status))
	{
		nxWriteToLogFile(L"nxrmdrv!DriverEntry: Leave, success");
		return status;
	}
...
__failed:
	...
	nxWriteToLogFile(L"nxrmdrv!DriverEntry: Leave, failed");
	return status;
}
...
{
	nxWriteToLogFile(L"nxrmdrv!DummyUnload: Enter");
	nxWriteToLogFile(L"nxrmdrv!DummyUnload: Leave");
}
	...
	)
{
	...
	case IOCTL_NXRMDRV_CHECK_DWM_HOOK_STATUS:
		{
			if (!g_bDWMAttached) {
