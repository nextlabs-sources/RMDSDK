	...
	PVOID						UserContext)
{
	...
	do 
	{
		...
		{
            if(NULL!=DebugDumpCheckLevelCallback && NULL!=DebugDumpCallback && DebugDumpCheckLevelCallback(NX::dbg::LL_WARNING))
            {
                NX::dbg::log_item le(NX::dbg::LL_WARNING, NX::dbg::build_error_msg(__FILE__, __FUNCTION__, __LINE__, NXMODNAME(), "Manager already exists."));
                const std::wstring& ls = le.serialize();
                DebugDumpCallback(NX::dbg::LL_WARNING, ls.c_str());
            }
			break;
		}
		...
		{
			bFailed = TRUE;
            if(NULL!=DebugDumpCheckLevelCallback && NULL!=DebugDumpCallback && DebugDumpCheckLevelCallback(NX::dbg::LL_ERROR))
            {
                NX::dbg::log_item le(NX::dbg::LL_ERROR, NX::dbg::build_error_msg(__FILE__, __FUNCTION__, __LINE__, NXMODNAME(), "Fail to allocate memory for Manager Object."));
                const std::wstring& ls = le.serialize();
                DebugDumpCallback(NX::dbg::LL_ERROR, ls.c_str());
            }
			break;
		}
		...
		{
			bFailed = FALSE;
            LOGMAN_WARN(hMgr, "CreateManager: Fail to init transporter server");
			break;
		}
		...
		{
			bFailed = TRUE;
            LOGMAN_ERROR(hMgr, "CreateManager: Fail to create stop event (%d)", GetLastError());
			break;
		}
		...
		{
			bFailed = TRUE;
            LOGMAN_ERROR(hMgr, "CreateManager: Fail to create worker thread (%d)", GetLastError());
			break;
		}
		...
		{
			bFailed = TRUE;
            LOGMAN_ERROR(hMgr, "CreateManager: Fail to create cache worker thread (%d)", GetLastError());
			break;
		}
...
//following define is only use for kill dwm.exe return
#define STATUS_RESTART_BOOT_APPLICATION  ((NTSTATUS)0xC0000453L)

BOOL nxrmdrvNeedRestartDWM(void)
{
	ULONG uRet = ERROR_SUCCESS;

	HANDLE hnxrmdrv = INVALID_HANDLE_VALUE;

	ULONG ulbytesret = STATUS_INVALID_PARAMETER;
	...
	do
	{
		...
		if (!DeviceIoControl(hnxrmdrv,
			IOCTL_NXRMDRV_CHECK_DWM_HOOK_STATUS,
			NULL,
			0,
			NULL,
			0,
	...
	//only return true when received STATUS_RESTART_BOOT_APPLICATION return code from driver. 
	//for legacy nxrmdrv, restart dwm.exe is done at driver level.
	if ((ERROR_SUCCESS == uRet && STATUS_RESTART_BOOT_APPLICATION == ulbytesret ) || ERROR_RESTART_APPLICATION == uRet){
			return TRUE;
	}
	return FALSE;
}
...
ULONG KillDwm(void)
{
	...
	do
	{
		if (wcsstr(pe32.szExeFile, L"dwm.exe")) {
			HANDLE hProcess;
...
ULONG	NXRMDRVMAN_API __stdcall nxrmdrvmanStart(NXRMDRV_HANDLE hMgr)
{
	...
	//following code to enforce restarting dwm.exe if needed
	if (nxrmdrvNeedRestartDWM()) {
		ULONG uErr;
		uErr = KillDwm();
		if (ERROR_SUCCESS != uErr) {
			LOGMAN_ERROR(hMgr, "Fail to restart dwm process (%d)", uErr);
		}
		else {
			LOGMAN_INFO(hMgr, "Restart dwm process.");
		}
	}
	...
	ULONG							ObligationsLength)
{
	...
	do 
	{
		...
		{
			uRet = ERROR_INVALID_PARAMETER;            
            LOGMAN_ERROR(hMgr, "Invalid message context (%d)", GetLastError());
			break;
		}
		...
		{
			uRet = ERROR_INVALID_PARAMETER;
            LOGMAN_ERROR(hMgr, "Mismatch message type (%d)", GetLastError());
			break;
		}
		...
		{
			uRet = ERROR_INVALID_PARAMETER;
            LOGMAN_ERROR(hMgr, "Fail to post response (%d)", GetLastError());
			break;
		}
	...
	PVOID							msgctx)
{
	...
	do 
	{
		...
		{
			uRet = ERROR_INVALID_PARAMETER;
            LOGMAN_ERROR(hMgr, "Invalid message context (%d)");
			break;
		}
		...
		{
			uRet = ERROR_INVALID_PARAMETER;
            LOGMAN_ERROR(hMgr, "Mismatch message type (%d)");
			break;
		}
		...
		{
			uRet = ERROR_INVALID_PARAMETER;
            LOGMAN_ERROR(hMgr, "Fail to post response");
			break;
		}
	...
	GET_CLASSIFY_UI_RESPONSE		*response)
{
	...
	do 
	{
		...
		{
			uRet = ERROR_INVALID_PARAMETER;
            LOGMAN_ERROR(hMgr, "Invalid message context");
			break;
		}
		...
		{
			uRet = ERROR_INVALID_PARAMETER;
            LOGMAN_ERROR(hMgr, "Mismatch message type");
			break;
		}
...
DWORD WINAPI nxrmdrvProcessCacheWorker(_In_ LPVOID lpParameter)
{
	...
	while(!bstop)
	{
		...
		case WAIT_OBJECT_0+1:
			...
			{                
                LOGMAN_ERROR(pMgr, "Can not query the size of current loader snapshot (%d)", GetLastError());
			}
	...
	ULONG						*cbSize)
{
	...
	do
	{
		...
		if (get_dwm_active_session(pMgr->nxrmdrvsection) == SessionId)
		{
			...
			{
				LOGMAN_INFO(pMgr, "Adding Overlay Wnd[%d]=%x", i, hWnd[i]);
			}
#endif
														...
														PVOID				userctx)
{
	...
	do 
	{
		...
		if (!pProtectedModeAppInfo)
		{
			LOGMAN_ERROR(pMgr, "Update protected mode app information failed due to out of memory");
		...
		{
			LOGMAN_ERROR(pMgr, "Fail to post Update protected mode app information response (%d)", GetLastError());
		}
													   ...
													   PVOID				userctx)
{
	...
	do 
	{
		...
		if (!post_response(pMgr->nxrmdrvsection, (NXRMRECORD*)msgctx, (PVOID)&resp, sizeof(resp)))
		{
			LOGMAN_ERROR(pMgr, "Fail to post query protected mode app information response (%d)", GetLastError());
		}
