...
VhdReadBlock(
            ...
            _Out_writes_bytes_(BytesToRead) PUCHAR Buffer,
...
VhdWriteBlock(
              ...
              _In_reads_bytes_(BytesToWrite) const UCHAR* Buffer,
        ...
        )
{
    ...
    __try {
        ...
        Status = NkAesDecrypt2(Key, 32, NULL, 0, VHDFMT_SECTOR_SIZE, Blob->Header.DiskKey, 64);
        if (!NT_SUCCESS(Status)) {
            try_return(Status);
        }

        if (16 != RtlCompareMemory(Blob->Header.UniqueId, Blob->Header.DiskKey + 32, 16)) {
            try_return(Status = STATUS_WRONG_PASSWORD);
        }

        if (VHD_FLAG_ENCRYPTION_ENABLED == (Blob->Header.Flags & VHD_FLAG_ENCRYPTION_ENABLED)) {
            // Check if content is encrypted
            Status = NkAesImportKey(Blob->Header.DiskKey, 32, &Instance->KeyHandle);
            if (!NT_SUCCESS(Status)) {
                try_return(Status);
            }
        }
            ...
            )
{
    ...
    __try {
        ...
        if(BooleanFlagOn(BlockState, BLOCK_STATE_ALLOCED) && BooleanFlagOn(BlockState, BLOCK_STATE_INITED)) {
            ...
            Ivec = ByteOffset.QuadPart - VHDFMT_BLOCK_START;
            ...
            // Decrypt Data
            if (NULL != Instance->KeyHandle && 0 != Iosb.Information) {
                NTSTATUS DecryptStatus = NkAesDecrypt(Instance->KeyHandle, NULL, Ivec, VHDFMT_SECTOR_SIZE, Buffer, (ULONG)Iosb.Information);
              ...
              )
{
    ...
    __try {
        ...
        if(!BooleanFlagOn(BlockState, BLOCK_STATE_INITED)) {
            ...
            // Encrypt Data
            if (NULL != Instance->KeyHandle) {
                Status = NkAesEncrypt(Instance->KeyHandle, NULL, Ivec, VHDFMT_SECTOR_SIZE, Cipher, VHDFMT_BLOCK_SIZE);
                if (!NT_SUCCESS(Status)) {
                    try_return(Status);
                }
            }
            ...
            Cipher = (PUCHAR)ExAllocatePoolWithTag(PagedPool, BytesToWrite, NXRMTMPTAG);
            if (NULL == Cipher) {
                try_return(Status = STATUS_INSUFFICIENT_RESOURCES);
            }

            RtlZeroMemory(Cipher, BytesToWrite);
            RtlCopyMemory(Cipher, Buffer, BytesToWrite);
            ...
            // Encrypt Data
            if (NULL != Instance->KeyHandle) {
                Status = NkAesEncrypt(Instance->KeyHandle, NULL, Ivec, VHDFMT_SECTOR_SIZE, Cipher, BytesToWrite);
                if (!NT_SUCCESS(Status)) {
                    try_return(Status);
                }
            }
            ...
            // Write
            Status = ZwWriteFile(Instance->FileHandle, NULL, NULL, NULL, &Iosb, (PVOID)Cipher, BytesToWrite, &ByteOffset, NULL);
            if(!NT_SUCCESS(Status)) {
                try_return(Status);
            }
        }
