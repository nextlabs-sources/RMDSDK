...
GetDosNameFromDriveLetter(
                          _Out_writes_bytes_(128) LPWSTR DosName,
...
VOID
GetNtNameFromNumber(
                    _Out_writes_bytes_(128) LPWSTR NtName,
                    _In_ LONG VolumeId
                    )
{
    ASSERT(VolumeId >= 0 && VolumeId < MAX_DISK_ID_VALUE);
                     ...
                     )
{
	...
	__try {
        ...
        do {
            ...
            (VOID)VhdFsctlCall(VolumeFileObject, FSCTL_UNLOCK_VOLUME, NULL, 0, NULL, 0);
                      ...
                      )
{
    ...
    *ppDeviceObject = NULL;
    ...
    ASSERT(DiskId < MAX_DISK_ID_VALUE);
              ...
              )
{
	...
	__try {
        ...
        Status = VhdOpen(&HostFileName, Mount->Key, &pExt->VhdFileHandle);
        if(!NT_SUCCESS(Status)) {
            try_return(Status);
        }
