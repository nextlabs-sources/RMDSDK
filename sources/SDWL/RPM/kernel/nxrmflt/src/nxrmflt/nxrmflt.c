	...
	)
{
    ...
    nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceSetup: Enter");
	...
	do
	{
...

        {
            ...
            swprintf_s(szBuffer, sizeof(szBuffer) / sizeof(wchar_t), L"nxrmflt!nxrmfltInstanceSetup Flags(%x), VolumeFilesystemType(%x)", Flags, VolumeFilesystemType);
            nxWriteToLogFile(szBuffer);
        }


		// We do not want to attach to volumes that are mounted after start-up
		// time.  This is such that we can avoid attaching to volumes mounted
		// by the user using virtual disk software like VeraCrypt which we
		// don't support.
		//
		// The side effect is that we also won't attach to a newly formatted
		// normal volume until after a reboot.
		if (FlagOn(Flags, FLTFL_INSTANCE_SETUP_NEWLY_MOUNTED_VOLUME) ||
			VolumeFilesystemType == FLT_FSTYPE_RAW)
		{
			status = STATUS_FLT_DO_NOT_ATTACH;

            nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceSetup: FlagOn or FLT_FSTYPE_RAW  STATUS_FLT_DO_NOT_ATTACH");
        ...
        {
            memset(szBuffer, 0, sizeof(szBuffer));
            swprintf_s(szBuffer, sizeof(szBuffer) / sizeof(wchar_t), L"nxrmflt!nxrmfltInstanceSetup FltAllocateContext status(%x)", status);
            nxWriteToLogFile(szBuffer);
        }
		...
		if (!NT_SUCCESS(status))
		{
            ...
            nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceSetup: FltAllocateContext  STATUS_FLT_DO_NOT_ATTACH");
        ...
        {
            memset(szBuffer, 0, sizeof(szBuffer));
            swprintf_s(szBuffer, sizeof(szBuffer) / sizeof(wchar_t), L"nxrmflt!nxrmfltInstanceSetup FltGetVolumeProperties status(%x)", status);
            nxWriteToLogFile(szBuffer);
        }
		...
		if (!NT_SUCCESS(status))
		{
            ...
            nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceSetup: FltGetVolumeProperties Failed to query volume properties");
		}
		...
		do 
		{
            ...
            {
                memset(szBuffer, 0, sizeof(szBuffer));
                swprintf_s(szBuffer, sizeof(szBuffer) / sizeof(wchar_t), L"nxrmflt!nxrmfltInstanceSetup FltGetDiskDeviceObject status(%x)", status);
                nxWriteToLogFile(szBuffer);
            }
			...
			if (!NT_SUCCESS(status))
			{
                ...
                nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceSetup: Failed to get volume device object! Disable filtering.");
            ...
            {
                memset(szBuffer, 0, sizeof(szBuffer));
                swprintf_s(szBuffer, sizeof(szBuffer) / sizeof(wchar_t), L"nxrmflt!nxrmfltInstanceSetup IoVolumeDeviceToDosName status(%x)", status);
                nxWriteToLogFile(szBuffer);
            }
			...
			if (!NT_SUCCESS(status))
			{
                ...
                nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceSetup: Failed to query volume DOS device name!");
            ...
            {
                memset(szBuffer, 0, sizeof(szBuffer));
                swprintf_s(szBuffer, sizeof(szBuffer) / sizeof(wchar_t), L"nxrmflt!nxrmfltInstanceSetup nxrmfltGetDeviceInfo status(%x)", status);
                nxWriteToLogFile(szBuffer);
            }
			...
			if (!NT_SUCCESS(status))
			{
                ...
                nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceSetup: Failed to get device information!");

        //
        // fix for bajaj issue, GetDeviceInfo of some machine's disk device returns STATUS_INVALID_DEVICE_REQUEST
        // as we can't get bus type for these device, we assume it is not USB or 1394, and by default allow our filter drive take effects on these devices
        //
        ...
        break;
            ...
            {
                memset(szBuffer, 0, sizeof(szBuffer));
                swprintf_s(szBuffer, sizeof(szBuffer) / sizeof(wchar_t), L"nxrmflt!nxrmfltInstanceSetup InstCtx->BusType(%x)", InstCtx->BusType);
                nxWriteToLogFile(szBuffer);
            }
				...
				InstCtx->BusType == BusType1394)
			{
                ...
                {
                    memset(szBuffer, 0, sizeof(szBuffer));
                    swprintf_s(szBuffer, sizeof(szBuffer) / sizeof(wchar_t), L"nxrmflt!nxrmfltInstanceSetup Device %s is removable device. Disable filtering", InstCtx->DeviceName);
                    nxWriteToLogFile(szBuffer);
                }
        ...
        InitializeListHead(&InstCtx->SafeDirList);
#ifdef  NEXTLABS_FEATURE_SKYDRM_SANCTUARY_DIR
        InitializeListHead(&InstCtx->SanctuaryDirList);
#endif  // NEXTLABS_FEATURE_SKYDRM_SANCTUARY_DIR
        ExInitializeResourceLite(&InstCtx->SafeDirListLock);
    ...
    nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceSetup: Leave");
	...
	)
{
	...
	nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceQueryTeardown: Enter");
	nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceQueryTeardown: Leave");
...
{
	nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceTeardownStart: Enter");
	NXRMFLT_INSTANCE_CONTEXT *InstCtx = NULL;
	NTSTATUS Status = STATUS_SUCCESS;
	...
	do
	{
		if (FlagOn(Flags, FLTFL_INSTANCE_TEARDOWN_MANUAL))
		{
			Status = STATUS_FLT_DO_NOT_DETACH;

            nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceTeardownStart: FlagOn FLTFL_INSTANCE_TEARDOWN_MANUAL");
		...
		if (!NT_SUCCESS(Status))
		{
			...
			nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceTeardownStart: Leave, Instance Context is missing");
			return;
		}
	...
	nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceTeardownStart: Leave");
}
	...
	)
{
	...
	nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceTeardownComplete: Enter");
	nxWriteToLogFile(L"nxrmflt!nxrmfltInstanceTeardownComplete: Leave");
}
	...
	)
{
	...
	if (NT_SUCCESS(status)) 
	{
		...
		nxSetLogFileName(L"nxrmflt.log");
	...
	)
{
	...
	nxWriteToLogFile(L"nxrmflt!nxrmfltUnload: Enter");
	...
	do 
	{
		...
		RB_EACH_NODE(ite, &Global.NxlFileCache)
		{
			...
			Status = nxrmfltDeleteFileByNameSync(pNode->Instance, &pNode->ReparseFileName);
	...
	nxWriteToLogFile(L"nxrmflt!nxrmfltUnload: Leave");
	return Status;
}
	...
	)
{
	...
	do
	{
		if (ContextType == FLT_STREAM_CONTEXT)
		{
			...
			//When release stream context, doesn't delete the temp decrypted file.
...
{
	nxWriteToLogFile(L"nxrmflt!InstCtxCleanup: Enter");
	...
	do
	{
		...
		ExDeleteResourceLite(&InstCtx->SafeDirListLock);
		ExDeleteResourceLite(&InstCtx->Resource);

	} while (FALSE);

	nxWriteToLogFile(L"nxrmflt!InstCtxCleanup: Leave");
	return;
}
...
BOOLEAN InitializeGlobalData(void)
{
	...
	do
	{
		...
		ExInitializePagedLookasideList(&Global.SessionCacheLookaside,
									   NULL,
									   NULL,
									   0,
									   sizeof(NXL_SESSION_CACHE_NODE),
									   NXRMFLT_SESSIONCACHE_TAG,
									   0);

		ExInitializePagedLookasideList(&Global.TokenCacheLookaside,
									   NULL,
									   NULL,
									   0,
									   sizeof(NXL_TOKEN_CACHE_NODE),
									   NXRMFLT_TOKENCACHE_TAG,
									   0);

		ExInitializePagedLookasideList(&Global.OpenInfoLookaside,
									   NULL,
									   NULL,
									   0,
									   sizeof(NXL_OPEN_INFO),
									   NXRMFLT_OPENINFO_TAG,
									   0);
									   
		ExInitializePagedLookasideList(&Global.CreateInfoLookaside,
									   NULL,
									   NULL,
									   0,
									   sizeof(NXL_CREATE_INFO),
									   NXRMFLT_CREATEINFO_TAG,
									   0);

		ExInitializePagedLookasideList(&Global.DebugLogMsgBufLookaside,
									   NULL,
									   NULL,
									   0,
									   NXRMFLT_DEBUG_LOG_MSG_BUF_SIZE,
									   NXRMFLT_DEBUGLOGMSGBUF_TAG,
									   0);
		...
		InitializeListHead(&Global.SessionCache);

		FltInitializePushLock(&Global.SessionCacheLock);
...
void CleanupGlobalData(void)
{
	...
	FltDeletePushLock(&Global.SessionCacheLock);
	...
	ExDeletePagedLookasideList(&Global.SessionCacheLookaside);

	ExDeletePagedLookasideList(&Global.TokenCacheLookaside);

	ExDeletePagedLookasideList(&Global.OpenInfoLookaside);

	ExDeletePagedLookasideList(&Global.CreateInfoLookaside);

	ExDeletePagedLookasideList(&Global.DebugLogMsgBufLookaside);
...
)
{
	...
	nxWriteToLogFile(L"nxrmflt!nxrmfltPreShutdown: Enter");
	...
	nxWriteToLogFile(L"nxrmflt!nxrmfltPreShutdown: Leave");
	...
	)
{
	...
	else if(pNode == NULL && CreateInfo)
	{
		...
		if(pNode)
		{
            ...
            pNode->HideXNLExtension     = FALSE; // is_app_in_real_name_access_list(Process) ? FALSE : TRUE;
