...
#define NXRMFLT_DEBUG_LOG_MSG_BUF_SIZE			(1024)
...
#define NXRMFLT_DEBUGLOGMSGBUF_TAG			'3MYT'
...
#define NXRMFLT_SESSIONCACHE_TAG			'RMYT'
#define NXRMFLT_TOKENCACHE_TAG				'SMYT'
#define NXRMFLT_OPENINFO_TAG				'TMYT'
#define NXRMFLT_CREATEINFO_TAG				'UMYT'
...
//#define NXRMFLT_FLAG_NOT_ENCRYPTED		(0x0010)
...
typedef struct _NXRMFLT_SAFEDIR {
	LIST_ENTRY		Link;
	BOOLEAN			Overwrite;
	BOOLEAN			AutoAppendNxlExt;
	UNICODE_STRING	DirPath;
} NXRMFLT_SAFEDIR, *PNXRMFLT_SAFEDIR;

#ifdef  NEXTLABS_FEATURE_SKYDRM_SANCTUARY_DIR
typedef struct _NXRMFLT_SANCTUARYDIR {
	LIST_ENTRY		Link;
	UNICODE_STRING	DirPath;
} NXRMFLT_SANCTUARYDIR, *PNXRMFLT_SANCTUARYDIR;
...
typedef struct _NXRMFLT_INSTANCE_CONTEXT{
    ...
    LIST_ENTRY                      SafeDirList;
#ifdef  NEXTLABS_FEATURE_SKYDRM_SANCTUARY_DIR
    LIST_ENTRY                      SanctuaryDirList;
#endif  // NEXTLABS_FEATURE_SKYDRM_SANCTUARY_DIR
    ERESOURCE						SafeDirListLock;
...
typedef struct _NXRMFLT_GLOBAL_DATA{
	...
	PAGED_LOOKASIDE_LIST	SessionCacheLookaside;

	PAGED_LOOKASIDE_LIST	TokenCacheLookaside;

	PAGED_LOOKASIDE_LIST	OpenInfoLookaside;

	PAGED_LOOKASIDE_LIST	CreateInfoLookaside;

	PAGED_LOOKASIDE_LIST	DebugLogMsgBufLookaside;
	...
	LIST_ENTRY				SessionCache;

	EX_PUSH_LOCK			SessionCacheLock;
...
typedef struct _NXRMFLT_SETINFORMATION_CONTEXT{
	...
	BOOLEAN							bSrcIsInSafeDir;

	BOOLEAN							bDesIsInSafeDir;
...
typedef struct _NXL_SESSION_CACHE_NODE {
	...
	EX_PUSH_LOCK		TokenCacheLock;

	LIST_ENTRY			Link;

	ULONG				ExpireTick;

	LIST_ENTRY			Link2;	// so that we can add session cache nodes into temp list

} NXL_SESSION_CACHE_NODE, *PNXL_SESSION_CACHE_NODE;

typedef struct _NXL_TOKEN_CACHE_NODE {
	...
	LARGE_INTEGER		TokenTTL;

	rb_node				Node;

} NXL_TOKEN_CACHE_NODE, *PNXL_TOKEN_CACHE_NODE;