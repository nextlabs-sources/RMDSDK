	...
	do 
	{
		...
		if (0 == RtlCompareString(&EaName, &EncryptContentEa, FALSE) && Ctx == NULL)	// can not encrypt encrypted file. Encrypted file has Ctx
		{
			...
			if (EaValue)
			{
				...
				if (pNode)
				{
					Status = nxrmfltCreateEmptyNXLFile(FltInstance,
													   SessionId,
		...
		else if (0 == RtlCompareString(&EaName, &TagEa, FALSE))
		{
			...
			do 
			{
				...
				Status = NXLCheck(pCacheNode->Instance, NXLFileObject);
				...
				//	nxrmfltSendKeyChainErrorMsg(FltInstance, &pCacheNode->OriginalFileName, NULL);
				...
				Status = nxrmfltFetchIVSeedFromFile(FltInstance,
													&pCacheNode->OriginalFileName,
													IvSeed);
				...
				Status = NXLSetSectionData(FltInstance,
										   NXLFileObject,
										   Token.Token,
										   IvSeed,
										   Tag,
